...
import { getApiKey, saveApiKey, getVeniceKey, saveVeniceKey, getOpenRouterKey, saveOpenRouterKey, getRoutstrKey, saveRoutstrKey, getAIProvider, setAIProvider, getAnthropicModel, setAnthropicModel, getVeniceModel, setVeniceModel, getOpenRouterModel, setOpenRouterModel, getRoutstrModel, setRoutstrModel, getOllamaMainModel, setOllamaMainModel, getOllamaPIIModel, setOllamaPIIModel, getOllamaPIIUrl, setOllamaPIIUrl, validateApiKey, validateVeniceKey, validateOpenRouterKey, validateRoutstrKey, fetchAnthropicModels, fetchVeniceModels, fetchOpenRouterModels, fetchRoutstrModels, renderModelPricingHint, getAnthropicModelDisplay, getVeniceModelDisplay, getOpenRouterModelDisplay, getRoutstrModelDisplay } from './api.js';
    ...
    <div class="settings-tab-panel${_activeSettingsTab === 'ai' ? ' active' : ''}" data-tab-panel="ai">
      ...
      <div class="settings-section">
        ...
        <div class="ai-provider-toggle">
          ...
          <button class="ai-provider-btn${provider === 'openrouter' ? ' active' : ''}" data-provider="openrouter" onclick="switchAIProvider('openrouter')"><svg class="ai-provider-logo" viewBox="0 0 24 24" fill="currentColor"><path d="M4 4h6v6H4V4zm10 0h6v6h-6V4zM4 14h6v6H4v-6zm10 0h6v6h-6v-6z" opacity="0.3"/><path d="M3 3v8h8V3H3zm2 2h4v4H5V5zm8-2v8h8V3h-8zm2 2h4v4h-4V5zM3 13v8h8v-8H3zm2 2h4v4H5v-4zm8-2v8h8v-8h-8zm2 2h4v4h-4v-4z"/></svg> OpenRouter</button>
...
export function renderAIProviderPanel(provider) {
  ...
  if (provider === 'routstr') {
    ...
    const routstrModel = getRoutstrModel();
    const cachedRoutstrModels = JSON.parse(localStorage.getItem('labcharts-routstr-models') || '[]');
    let routstrModelHtml;
    if (cachedRoutstrModels.length > 0) {
      const opts = cachedRoutstrModels.map(function(m) {
        const label = m.name || m.id;
        return '<option value="' + m.id + '"' + (routstrModel === m.id ? ' selected' : '') + '>' + escapeHTML(label) + '</option>';
      }).join('');
      routstrModelHtml = `<div style="margin-top:12px" id="routstr-model-area">
        <label style="font-size:12px;color:var(--text-muted)">Model</label>
        ...
        <div id="routstr-model-pricing" style="margin-top:4px">${renderModelPricingHint('routstr', routstrModel)}</div>
      </div>`;
    } else {
      routstrModelHtml = `<div style="margin-top:12px;font-size:12px;color:var(--text-muted)" id="routstr-model-area">Model: <span style="color:var(--text-primary)">${escapeHTML(getRoutstrModelDisplay())}</span>${currentKey ? ' <span style="font-size:11px">(save key to load models)</span>' : ''}</div>`;
    }
    return `<div class="ai-provider-panel">
      <div class="ai-provider-desc">Decentralized AI inference. Pay anonymously with eCash or Lightning Bitcoin &mdash; no credit card, no account required.</div>
      <div class="api-key-status" id="routstr-key-status">
        ${currentKey ? '<span style="color:var(--green)">&#10003; Connected</span>' : '<span style="color:var(--text-muted)">No key set</span>'}
      </div>
      <input type="password" class="api-key-input" id="routstr-key-input" placeholder="sk-..." value="${currentKey}">
      <div style="display:flex;gap:8px;margin-top:12px">
      ...
      ${routstrModelHtml}
      <div class="api-key-notice">Your key is stored locally and sent directly to Routstr. <a href="https://routstr.com/" target="_blank" rel="noopener" style="color:var(--accent)">Get an API token</a><br><span style="font-size:11px;color:var(--text-muted)">Anonymous &middot; eCash / Lightning Bitcoin</span></div>
    </div>`;
  }
  if (provider === 'venice') {
...
export async function handleSaveRoutstrKey() {
  ...
  if (result.valid) {
    ...
    showNotification('Routstr API key saved', 'success');
  } else {
    status.innerHTML = `<span style="color:var(--red)">${result.error}</span>`;
  }
  btn.disabled = false; btn.textContent = 'Save & Validate';
...
export function handleRemoveRoutstrKey() {
  ...
  showNotification('Routstr API key removed', 'info');
  openSettingsModal();
}
