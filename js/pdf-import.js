...
import { callClaudeAPI, hasAIProvider, getAIProvider, getAnthropicModel, getVeniceModel, getOpenRouterModel, getRoutstrModel, getOllamaMainModel, getAnthropicModelDisplay, getVeniceModelDisplay, getOpenRouterModelDisplay, getRoutstrModelDisplay, getOllamaPIIModel } from './api.js';
...
export function showImportPreview(parseResult) {
  ...
  if (parseResult.costInfo) {
    ...
    const modelLabel = ci.provider === 'ollama' ? getOllamaMainModel() : ci.provider === 'venice' ? getVeniceModelDisplay() : ci.provider === 'openrouter' ? getOpenRouterModelDisplay() : ci.provider === 'routstr' ? getRoutstrModelDisplay() : getAnthropicModelDisplay();
  ...
  if (isDebugMode()) {
    ...
    if (t) {
      ...
      const provider = getAIProvider();
      const modelLabel = provider === 'ollama' ? getOllamaMainModel() : provider === 'venice' ? getVeniceModelDisplay() : provider === 'openrouter' ? getOpenRouterModelDisplay() : provider === 'routstr' ? getRoutstrModelDisplay() : getAnthropicModelDisplay();
...
export async function handlePDFFile(file) {
  ...
  try {
    ...
    const mid = prov === 'anthropic' ? getAnthropicModel() : prov === 'venice' ? getVeniceModel() : prov === 'openrouter' ? getOpenRouterModel() : prov === 'routstr' ? getRoutstrModel() : getOllamaMainModel();
    result.costInfo = {
