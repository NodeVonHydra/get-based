...
If you have lab results but no PDF — for example, values written on paper, a result emailed as plain text, or numbers from a health device — you can enter them directly into Lab Charts using the manual entry form.
...
Only enter the markers you have results for. Leave the rest blank. Lab Charts uses `null` for missing values and draws chart lines across gaps automatically.
:::
...
::: warning Units
Make sure you enter values in the units shown next to each marker. Lab Charts stores data in SI units (EU mode) by default. If your results are in US units (e.g., mg/dL for glucose instead of mmol/L), switch to **US units** in **Settings → Display** before entering values, or convert manually.
:::
