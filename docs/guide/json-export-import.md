...
Lab Charts can export all your data to a JSON file and import it back. This is useful for backups, transferring data between devices or browsers, and sharing data with another Lab Charts instance.
...
Lab Charts detects JSON files automatically and handles them differently from PDFs:
...
To move Lab Charts data to a new computer or browser:

1. Export JSON on the old device
2. Open Lab Charts on the new device
...
Lab Charts also creates automatic snapshots in the background as you add data. Up to 5 snapshots are kept, and you can restore any of them from **Settings → Backup & Restore**. These backups are stored in your browser's IndexedDB and do not leave your device.
