...
Installable via `manifest.json` + `service-worker.js`. Cache: `labcharts-v42` (bump to bust). Strategies: API/OpenRouter/Routstr/Ollama → bypass SW entirely (no `event.respondWith`, avoids IPC stream buffering), Google Fonts → stale-while-revalidate, CDN → cache-first, app shell → stale-while-revalidate.
