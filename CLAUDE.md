...
- **Data structures**:
  ...
  - `sleepRest` = `{ duration, quality, schedule, roomTemp, issues[], environment[], practices[], note }` — circadian-informed: room temperature, sleep environment (blackout, EMF, grounding sheet, magnetico), practices (mouth taping, cold shower, magnesium)
  - `lightCircadian` = `{ amLight, daytime, uvExposure, evening[], screenTime, techEnv[], cold, grounding, mealTiming[], note }` — circadian-informed: AM light, UV exposure, evening discipline, cold exposure, grounding/earthing, screen time, technology environment, meal timing. Latitude auto-detected from Settings location
  ...
  - `environment` = `{ setting, climate, water, waterConcerns[], emf[], emfMitigation[], homeLight, air[], toxins[], building, note }` — circadian-informed: water quality (spring, DDW, RO), EMF sources + mitigation, home lighting, air quality, toxin exposure, building materials
...
- **Stop button**: `_chatAbortController` (module-level) manages streaming abort. `sendChatMessage()` toggles — if called during streaming, aborts and returns. Send button switches between send mode (➤, accent) and stop mode (■, red `.streaming` class). Aborted responses save partial text with `stopped: true` flag and `[stopped]` note. Signal passed through `callClaudeAPI` → all 4 providers via `AbortSignal.any([signal, AbortSignal.timeout(300000)])`
- **Discuss button**: `startDiscussion()` auto-alternates between personas when 2+ have responded in a thread. `getThreadPersonaCount()` counts unique `personalityName` values. `updateDiscussButton()` shows/hides `#chat-discuss-btn`. Each persona gets one turn per round: auto user message (`auto: true`) + streamed AI response. After each round, inline "Continue discussion?" / "Done" buttons appear (`showDiscussContinuePrompt`). `continueDiscussion()` runs another round, `endDiscussion()` cleans up. Stop button works mid-discussion. Original personality restored after. Auto messages render with `.chat-msg-auto` class (smaller, muted, italic, centered). Sending a manual message clears discussion state
...
- **OpenRouter**: OpenAI-compatible API marketplace (CORS-enabled) routing to 200+ models. Via `callOpenAICompatibleAPI` with attribution headers. Model from `getOpenRouterModel()` (default: `anthropic/claude-sonnet-4-6`). Key: `labcharts-openrouter-key`. Model IDs use `provider/model-name` format (e.g., `anthropic/claude-sonnet-4-6`). **Curated model list**: `OPENROUTER_CURATED` whitelist of latest-gen medically capable models (Claude 4.6, GPT-5.2, Gemini 3.1 Pro/3 Flash, DeepSeek v3.2, Qwen 3.5/3 Max, Grok 4) — prefix-matched against model IDs. `OPENROUTER_EXCLUDE` blocklist filters codex/audio/image/oss/safeguard/coder variants. **Dynamic pricing**: `fetchOpenRouterModels()` extracts `pricing.prompt`/`pricing.completion` (per-token strings → per-million-token floats) from API response, cached in `labcharts-openrouter-pricing`. `getOpenRouterPricing(modelId)` reads cache; `getModelPricing()` in schema.js checks dynamic cache first for OpenRouter, falls back to `_default: { input: 1.00, output: 3.00, approx: true }`. Models cached in `labcharts-openrouter-models`. **OAuth PKCE login**: `generatePKCE()` creates code_verifier + SHA-256 code_challenge via Web Crypto. `startOpenRouterOAuth()` stores verifier in `sessionStorage('or_pkce_verifier')`, redirects to `openrouter.ai/auth` with callback_url + code_challenge. On return, `main.js` detects `?code=` param, `exchangeOpenRouterCode(code)` POSTs to `openrouter.ai/api/v1/auth/keys` to get a permanent API key. Settings shows "Connect with OpenRouter" button (`.or-oauth-btn`) + "or enter key manually" divider (`.or-oauth-divider`) when no key exists; hidden when connected. Constraint: `callback_url` must be HTTPS or `http://localhost:3000`
